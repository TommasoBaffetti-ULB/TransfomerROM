...
class CAEloss(Loss):
    def __init__(self, type:Literal["MSE","MAE","Huber","SmoothL1"]="MSE", lambda_phy=0.1, lambda_t1:float=0.7, lambda_t2:float=0.3, reduction: Literal["mean", "sum"] = "mean", phys=True) -> None:
    ...
    def forward(self, prediction: torch.Tensor, target: torch.Tensor, **kwargs):
        ...
        phyloss=self.phyloss(prediction=prediction, target=target,**kwargs)
        return loss_t, self.lambda_t2 * loss_m + self.lambda_t1 * loss_t[0] + self.lambda_phy * phyloss
